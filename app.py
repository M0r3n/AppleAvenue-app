...
# ── КАСТОМНЫЙ CSS ДЛЯ ВЫРАВНИВАНИЯ ЗАГОЛОВКОВ ЭКСПАНДЕРОВ ─────────────────────
# Этот блок заставляет заголовок экспандера работать как flexbox,
# расталкивая содержимое (текст и квадраты) по краям.
st.markdown("""
<style>
    /* Таргетируемся на контейнер внутри заголовка экспандера */
    ...
    /* Убираем стандартные отступы markdown, чтобы flex работал чисто */
    .streamlit-expanderHeader div[data-testid="stMarkdownContainer"] {
        width: 100%;
    }
</style>
...
def get_warehouse_squares(group: pd.DataFrame) -> str:
    ...
    if "пекин" in all_whs:
        squares.append("🟩")
    # Оборачиваем в span для CSS, если нужно точечное управление,
    # но flexbox сработает и так.
    return "".join(squares)

def build_order_header(oid: str, tags: list, squares: str) -> str:
    """Собирает структуру заголовка для flexbox выравнивания."""
    tag_str = f" | {' | '.join(tags)}" if tags else ""
    # Мы возвращаем строку, которую CSS превратит во flex-контейнер.
    # Левая часть (текст) и правая часть (squares) будут разнесены по краям.
    return f"<span>Заказ №{oid}{tag_str}</span> <span>{squares}</span>"
...
def render_store(current_store: str) -> None:
    ...
    with col1:
        ...
        for oid, group in new_items.groupby(C_ORDER, sort=False):
            ...
            # Теперь заголовок содержит HTML-структуру, которую отформатирует CSS
...
if "Магазин" in menu:
    ...
    for oid, group in moves.groupby(C_ORDER, sort=False):
        squares = get_warehouse_squares(group)
        # Применяем ту же структуру заголовка
