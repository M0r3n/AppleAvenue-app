...
# ── CSS ДЛЯ ВЫРАВНИВАНИЯ КВАДРАТОВ ПО ПРАВОМУ КРАЮ ────────────────────────────
st.markdown("""
<style>
    /* Находим параграф внутри заголовка экспандера */
    ...
    /* Убираем стандартные отступы, чтобы выравнивание было по краям */
...
def build_header_with_squares(oid, tags_str, squares):
    """Сборка заголовка для CSS выравнивания."""
    # Оборачиваем левую и правую части в span, чтобы CSS растолкал их
    return f"<span>Заказ №{oid}{tags_str}</span> <span>{squares}</span>"
...
def render_store(current_store: str) -> None:
    ...
    with col1:
        ...
        for oid, group in new_items.groupby(C_ORDER, sort=False):
            ...
            # Формируем заголовок (CSS растолкает span-ы по краям)
...
if "Магазин" in menu:
    ...
    for oid, group in moves.groupby(C_ORDER, sort=False):
        squares = get_warehouse_squares(group)
        # В заголовке перемещений тоже выравниваем
