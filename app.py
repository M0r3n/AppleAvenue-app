...
# ── КАСТОМНЫЙ CSS ДЛЯ МАКСИМАЛЬНОГО СМЕЩЕНИЯ ВПРАВО ───────────────────────────
st.markdown("""
<style>
    /* Настройка контейнера заголовка экспандера */
    ...
    /* Класс для левой части (текст заказа), который забирает всё место */
    .order-text-left {
        flex-grow: 1;
        text-align: left;
    }
    
    /* Класс для правой части (квадраты) */
    .order-squares-right {
        flex-shrink: 0;
        text-align: right;
        margin-right: 15px; /* небольшой отступ от иконки стрелочки */
    }

    /* Убираем лишние отступы самого контейнера */
...
def build_order_header(oid: str, tags: list, squares: str) -> str:
    """Собирает заголовок с использованием HTML-классов для CSS-выравнивания."""
    tag_str = f" | {' | '.join(tags)}" if tags else ""
    # Мы упаковываем части в span с классами, которые описаны в блоке CSS выше
    return f'<span class="order-text-left">Заказ №{oid}{tag_str}</span> <span class="order-squares-right">{squares}</span>'
