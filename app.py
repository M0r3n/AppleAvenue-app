...
# CSS для выравнивания: текст слева, квадраты справа
st.markdown("""
<style>
    ...
    .header-right { flex: 0; text-align: right; white-space: nowrap; margin-left: 20px; }
</style>
...
def build_header(oid, tags_list, squares):
    """Собирает заголовок с разделением сторон."""
    tag_str = f" | {' | '.join(tags_list)}" if tags_list else ""
    return f'<span class="header-left">Заказ №{oid}{tag_str}</span><span class="header-right">{squares}</span>'
...
def render_store(current_store: str) -> None:
    ...
    with col1:
        ...
        for oid, group in new_items.groupby(C_ORDER, sort=False):
            ...
            header_label = build_header(oid, tags, squares)
