...
# CSS для выравнивания: текст и иконки слева, квадраты максимально справа
st.markdown("""
<style>
    ...
    .header-left { flex: 1; text-align: left; font-weight: 600; }
    .header-right { flex: 0; text-align: right; white-space: nowrap; margin-left: 20px; font-size: 1.2rem; }
</style>
...
def get_warehouse_squares(group: pd.DataFrame) -> str:
    """Генерирует чистые квадраты складов заказа."""
...
def build_header(oid, icons_list, squares):
    """Собирает лаконичный заголовок без лишних слов."""
    icon_str = f" | {' | '.join(icons_list)}" if icons_list else ""
    return f'<span class="header-left">{oid}{icon_str}</span><span class="header-right">{squares}</span>'
